...
import type { BillRate, Grade } from '@/lib/types';
...
export default function BillRatesPage() {
  const r = useResource<BillRate, Input, UpdateInput>('bill-rates');
  ...
  return (
    <AdminShell
      title="Bill Rates"
      description="Time-versioned client day rate per grade. T&M revenue = billable hours × bill rate effective on each log date."
      actions={
        <CreateDialog
          grades={grades.data ?? []}
      ...
      <div className="rounded-md border bg-card">
        <Table>
          ...
          <TableBody>
            ...
            {r.list.data?.length === 0 && <TableEmpty colSpan={5}>No bill rates yet.</TableEmpty>}
            {r.list.data?.map((c) => {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        >
          <DialogHeader>
            <DialogTitle>New bill rate</DialogTitle>
          </DialogHeader>
...
function EditDialog({
  ...
  return (
    ...
    >
      <DialogTrigger asChild>
        <Button variant="ghost" size="icon" aria-label="Edit bill rate">
      ...
      <DialogContent>
        ...
        >
          <DialogHeader>
            <DialogTitle>Edit bill rate</DialogTitle>
          </DialogHeader>
          <Field label="Grade">
            <Select
              value={form.watch('gradeId')}
